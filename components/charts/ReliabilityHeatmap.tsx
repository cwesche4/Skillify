    ...
    <div className="h-64 w-full">
      <ResponsiveContainer>
        <ComposedChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 4 }}>
          ...
          <Customized
            component={(props) => {
              const { xAxisMap, yAxisMap, offset, chartHeight, chartWidth, data } =
                props as any
              ...
              return (
                <>
                  {data.map((point: ReliabilityPoint, index: number) => {
                    ...
                    const cellWidth =
                      (chartWidth - offset.left - offset.right) / xAxis.ticks.length

                    const cellHeight =
                      (chartHeight - offset.top - offset.bottom) / yAxis.ticks.length
