...
import {
  ...
  createGoogleWatchChannelId,
} from '@/lib/scheduling/providers/google'
...
const GOOGLE_PROVIDER = PrismaCalendarProvider.GOOGLE
...
function providerEventToSchedulingInput(providerEvent: ProviderEventPayload) {
  return {
    title: providerEvent.title || 'Untitled Google Calendar event',
...
export async function listGoogleCalendarIntegrationState({
  workspaceId,
  ...
  const providerConfig = getSchedulingProviderConfig('google')
  ...
  if (
    ...
    if (existingMemberConnection) {
      return {
        ...
        safeMessage:
          'This workspace allows only one personal Google Calendar account per member.',
        retryable: false,
      }
    }
  }
...
export async function refreshGoogleCalendarConnection({
  ...
  const connection = await prisma.calendarConnection.findFirst({
    where: { id: connectionId, workspaceId, provider: GOOGLE_PROVIDER },
    include: {
  ...
  if (!connection) {
    return {
      ok: false as const,
      code: 'notFound',
      safeMessage: 'Google Calendar connection was not found.',
      retryable: false,
    }
  }
  const workspacePolicy =
  ...
  if (!eligibility.canDiscoverCalendars) {
    return {
      ...
      safeMessage:
        'This Google Calendar connection is not eligible for token refresh.',
      retryable: false,
    }
  }
  const tokens = await loadConnectionTokens(connection)
  const result = await createGoogleCalendarProvider().refreshToken({
    workspaceId,
    connectionId,
    ...tokens,
  })
...
export async function completeGoogleCalendarOAuth({
  ...
  if (!pending?.oauthState || !safeCompare(pending.oauthState, state)) {
    return {
      ok: false as const,
      code: 'invalidState',
      safeMessage: 'Google Calendar OAuth state could not be verified.',
      retryable: false,
    }
  }
  if (pending.oauthStateExpiresAt && pending.oauthStateExpiresAt < new Date()) {
    return {
      ...
      code: 'stateExpired',
      safeMessage:
        'Google Calendar OAuth state expired. Reconnect and try again.',
      retryable: false,
    }
  }
  const membershipId =
  ...
  if (!membershipId) {
    return {
      ...
      safeMessage:
        'Google Calendar OAuth state is missing workspace member ownership.',
      retryable: false,
    }
  }
  const membership = await prisma.workspaceMember.findFirst({
  ...
  if (!membership) {
    return {
      ...
      safeMessage:
        'The member who started this Google Calendar connection no longer belongs to the workspace.',
      retryable: false,
    }
  }
  const workspacePolicy =
...
export async function discoverGoogleCalendars({
  ...
  if (!eligibility.canDiscoverCalendars) {
    return {
      ...
      code: eligibility.reasonCode,
      safeMessage:
        'This Google Calendar connection is not eligible for calendar discovery.',
      retryable: false,
    }
  }
  const tokens = await loadConnectionTokens(connection)
  const result = await createGoogleCalendarProvider().listCalendars({
    workspaceId,
    connectionId,
    ...tokens,
  })
...
export async function updateConnectedGoogleCalendar({
  workspaceId,
  calendarId,
  selectedForSync,
  syncDirection,
  ...
  if (!calendar) {
    return {
      ok: false as const,
      code: 'notFound',
      safeMessage: 'Google Calendar was not found.',
      retryable: false,
    }
  }
...
export async function syncGoogleCalendarConnection({
  ...
  if (!eligibility.canPull && !eligibility.canPush) {
    await recordCalendarSyncLog({
      ...
      safeMessage: `Google Calendar sync skipped: ${eligibility.reasonCode}.`,
      workerId,
      startedAt,
          ...
          if (personalCalendar && providerEvent.id) {
            ...
            await prisma.calendarEventMapping.upsert({
              ...
              create: {
                ...
                syncHash,
                lastSyncOrigin: 'google',
                originOperation: 'google.pull.external_availability',
          ...
          if (
            ...
            await prisma.calendarEventMapping.update({
              ...
              data: {
                ...
                lastPulledAt: new Date(),
                lastSyncOrigin: 'google',
                originOperation: 'google.pull.loop_ignored',
              },
            })
            processed += 1
            continue
  ...
  await recordCalendarSyncLog({
    ...
    safeMessage:
      failures > 0
        ? `${failures} Google Calendar sync operations need attention.`
        : `Processed ${processed} Google Calendar sync records.`,
    workerId,
    startedAt,
    completedAt,
...
export async function renewGoogleCalendarWatchChannels({
  ...
  let renewed = 0
  for (const calendar of connection.calendars) {
    const channelId = createGoogleWatchChannelId()
...
export async function recordGoogleWebhookNotification({
  channelId,
  resourceId,
  messageNumber,
  resourceState,
  ...
  if (!channel) {
    return {
      ok: false as const,
      code: 'unknownChannel',
      safeMessage: 'Google Calendar webhook channel was not recognized.',
      retryable: false,
    }
  }
  const connection = await prisma.calendarConnection.findFirst({
  ...
  if (!connection) {
    return {
      ...
      safeMessage: 'Google Calendar webhook connection was not found.',
      retryable: false,
    }
  }
  ...
  if (!eligibility.canPull) {
    return {
      ok: false as const,
      code: eligibility.reasonCode,
      safeMessage: `Google Calendar webhook delivery was ignored: ${eligibility.reasonCode}.`,
      retryable: false,
    }
  }
    ...
    return {
      ...
      safeMessage:
        decision.reason === 'resourceMismatch'
          ? 'Google Calendar webhook resource did not match the channel.'
          : `Google Calendar webhook delivery was ignored: ${decision.reason}.`,
...
export async function previewGoogleCalendarInitialSync({
  workspaceId,
  connectionId,
}: {
  workspaceId: string
  ...
  if (!eligibility.canPull) {
    return {
      ...
      code: eligibility.reasonCode,
      safeMessage:
        'This Google Calendar connection is not eligible for import preview.',
      retryable: false,
    }
  }
  const tokens = await loadConnectionTokens(connection)
...
export async function resolveGoogleCalendarConflict({
  ...
  conflictId: string
  actorUserId: string
  resolution: 'keepSkillify' | 'keepGoogle' | 'merge' | 'ignore' | 'retryLater'
  rememberDecision?: boolean
}) {
  const conflict = await prisma.calendarSyncConflict.findFirst({
    where: { id: conflictId, workspaceId, provider: GOOGLE_PROVIDER },
  ...
  if (!conflict) {
    return {
      ok: false as const,
      code: 'notFound',
      safeMessage: 'Google Calendar conflict was not found.',
      retryable: false,
    }
  }
    ...
    if (conflict.mappingId) {
      await tx.calendarEventMapping.update({
        ...
        data: {
          ...
          lastSyncOrigin: resolution === 'keepSkillify' ? 'skillify' : 'google',
...
export async function inspectGoogleCalendarMappingIntegrity({
  workspaceId,
  repair = false,
}: {
  workspaceId: string
  ...
  await prisma.calendarSyncDiagnostic.create({
    data: {
      workspaceId,
      workerKind: 'google.mappingIntegrity',
      ...
      safeMessage:
        findings.length > 0
          ? `${findings.length} Google Calendar mapping integrity findings.`
          : 'Google Calendar mappings passed integrity checks.',
