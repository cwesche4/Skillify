...
import { getInspectorTabHeatmap } from '@/lib/analytics/inspectorAggregates'
...
export async function GET(req: Request) {
  ...
  const events =
    (await client.inspectorTelemetryAggregate.findMany({
      where: { workspaceId, bucket: bucket ?? 'day', kind: 'tab' },
      select: { payload: true },
    })) || []

  const normalized = getInspectorTabHeatmap(
