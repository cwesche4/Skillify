...
import { getInspectorValidationHeatmap } from '@/lib/analytics/inspectorAggregates'
...
export async function GET(req: Request) {
  ...
  const events =
    (await client.inspectorTelemetryAggregate.findMany({
      where: { workspaceId, bucket: bucket ?? 'day', kind: 'validation' },
      select: { payload: true },
    })) || []

  const normalized = getInspectorValidationHeatmap(
