...
import { renewMicrosoftCalendarWatchChannels } from '@/lib/scheduling/providers/microsoftService'
import {
  ...
  return withSchedulingWorkerAuth(request, async () => {
    ...
    const webhookUrl =
      ...
      (process.env.NEXT_PUBLIC_APP_URL
        ? `${process.env.NEXT_PUBLIC_APP_URL}/api/scheduling/microsoft/webhook`
        : null)
    if (!webhookUrl) {
      return Response.json(
        {
          ...
          message: 'Microsoft Outlook webhook URL is not configured.',
        },
        { status: 503 },
      )
    }
    ...
    for (const connection of connections) {
      results.push(
        await renewMicrosoftCalendarWatchChannels({
